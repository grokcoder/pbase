...
public class HRegionFileSystem {
  ...
  private static void sleepBeforeRetry(String msg, int sleepMultiplier, int baseSleepBeforeRetries,
     ...
     * This methods performs the filtering based on the valid store files.
     * @param familyName Column Family Name
     * @return a set of {@link parquet.hadoop.ParquetFileInfo} for the specified family.
     */
    public Collection<ParquetFileInfo> getPStoreFiles(final String familyName, final boolean validate)
            throws IOException {
        Path familyDir = getStoreDir(familyName);
        FileStatus[] files = FSUtils.listStatus(this.fs, familyDir);
        if (files == null) {
        ...
        ArrayList<ParquetFileInfo> storeFiles = new ArrayList<ParquetFileInfo>();
        for (FileStatus status: files) {
            if(status.getPath().getName().endsWith(".parquet")) {
               /* if (validate && !StoreFileInfo.isValid(status)) {
                    LOG.warn("Invalid StoreFile: " + status.getPath());
                ...
                ParquetFileInfo parquetFileInfo = new ParquetFileInfo(status.getPath());

                storeFiles.add(parquetFileInfo);
            }
        }
        return storeFiles;
    }
