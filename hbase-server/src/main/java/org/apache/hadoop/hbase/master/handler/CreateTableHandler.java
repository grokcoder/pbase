...
public class CreateTableHandler extends EventHandler {
  ...
  protected void addRegionsToMeta(final List<HRegionInfo> regionInfos)
      throws IOException {
    /**
     * argliy implementation, change it later !!
     * @author Wang Xiaoyi
     */
    String schema = this.hTableDescriptor.getValue("schema");
    LOG.info("########################### SCHEMA : " + this.hTableDescriptor.getValue("schema"));
    if(schema != null)
      MetaTableAccessor.addRegionsToMeta(this.server.getConnection(), regionInfos, schema);
    else
