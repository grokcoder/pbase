...
public class Put extends Mutation implements HeapSize, Comparable<Row> {
    ...
    public Put mergePut(Put put)throws IOException{
        if(! Bytes.equals(getRow(), put.getRow())){
            LOG.error("put merge error! " + "rowkey not equal");
            return this;
        }
        CellScanner scanner = put.cellScanner();
        while (scanner.advance()){
            Cell cell = scanner.current();
            List<Cell> cells = this.get(cell.getFamily(), cell.getQualifier());
            if(cells == null || cells.isEmpty())
                this.addColumn(cell.getFamily(), cell.getQualifier(), cell.getValue());
            else {
                cells.clear();
                cells.add(cell);
            }
