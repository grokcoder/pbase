...
import org.apache.hadoop.hbase.exception.SchemaNotFoundException;
   ...
   */
    ...
    private static Put addRegionInfo(final Put p, final HRegionInfo hri)
        ...
        p.addImmutable(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER,
                hri.toByteArray());
        //String schema = new String();//add the schema of table this hregion holds
        //p.addImmutable(HConstants.CATALOG_FAMILY, HConstants.SCHEMA_QUALIFIER, Bytes.toBytes(schema));
        return p;
    }
    ...
    /**#########################   functions for parquet   ###############################*/


    /**
     * Generates and returns a Put containing the region into for the catalog table
     * @param schema table schema info
     * @author Wang Xiaoyi
     */
    public static Put makePutFromRegionInfo(HRegionInfo regionInfo, String schema)
            throws IOException {
        Put put = new Put(regionInfo.getRegionName());
        addRegionInfo(put, regionInfo, schema);
        return put;
    }
    ...
    /**
     * add region info to meta encapusulated in put
     * @author Wang Xiaoyi
     * @param p
     * @param hri
     ...
     * @throws IOException
     */
    private static Put addRegionInfo(final Put p, final HRegionInfo hri, String schema)
            throws IOException {
        p.addImmutable(HConstants.CATALOG_FAMILY, HConstants.REGIONINFO_QUALIFIER,
                hri.toByteArray());
        p.addImmutable(HConstants.CATALOG_FAMILY, HConstants.SCHEMA_QUALIFIER, Bytes.toBytes(schema));
    ...
    public static String getTableSchema(TableName tableName){
        String schema = null;
        //Get get = new Get();
        return schema;
    }
     ...
     */
    public static String getSchemaForRegion(final HRegionInfo hri, final Connection connection)
        throws IOException, SchemaNotFoundException{

        String schema = null;
        Get get = new Get(hri.getRegionName());
        Result rs = get(connection.getTable(hri.getTable()), get);
        List<Cell> cells = rs.getColumnCells(HConstants.CATALOG_FAMILY, HConstants.SCHEMA_QUALIFIER);
        if(cells.size() > 0){
            schema = Bytes.toString(cells.get(cells.size() - 1).getValueArray());
        }else {
            LOG.error(hri.getRegionName()+" region schema not found!");
